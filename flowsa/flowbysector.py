...
def main(**kwargs):
    ...
    for primary_source, primary_config in fb.items():
        ...
        flows = load_source_dataframe(method, primary_source, primary_config,
                                      download_FBA_if_missing, fbsconfigpath)

        if primary_config['data_format'] == 'FBA':
            ...
            if callable(fxn):
                vLog.info(f"Cleaning up {primary_source} FlowByActivity")
                flows = fxn(fba=flows, source_dict=primary_config)
            elif fxn:
                raise flowsa.exceptions.FBSMethodConstructionError(
                    error_type='fxn_call')
            ...
            flows_mapped, mapping_files = (map_fbs_flows(
                flows, primary_source, primary_config, keep_fba_columns=True,
                keep_unmapped_rows=primary_config.get("keep_unmapped_rows", False)))

            # clean up fba, if specified in yaml
            fxn = primary_config.get("clean_fba_df_fxn")
            if callable(fxn):
                vLog.info(f"Cleaning up {primary_source} FlowByActivity")
                flows_mapped = fxn(fba=flows_mapped, source_dict=primary_config)
            elif fxn:
                raise flowsa.exceptions.FBSMethodConstructionError(
                    error_type='fxn_call')
            ...
            activities = primary_config['activity_sets']
            ...
            for aset, attr in activities.items():
                ...
                vLog.info(f"Preparing to handle {aset} in {primary_source}")
                ...
                if activity_geoscale != 'national':
                    compare_geographic_totals(
                        flows_subset_geo, flows_mapped, primary_source,
                        activity_geoscale, aset, names)

                # Add sectors to df activity, depending on level
                # of specified sector aggregation
                log.info("Adding sectors to %s", primary_source)
                ...
                if "clean_fba_w_sec_df_fxn" in primary_config:
                    vLog.info("Cleaning up %s FlowByActivity with sectors",
                              primary_source)
                    flows_subset_wsec = primary_config["clean_fba_w_sec_df_fxn"](
                        flows_subset_wsec,
                        attr=attr,
                        method=method
                    )
                ...
                compare_fba_geo_subset_and_fbs_output_totals(
                    flows_subset_geo, fbs_sector_subset, aset,
                    primary_source, primary_config, attr, method)

                log.info(f"Completed flowbysector for {aset}")
                fbs_list.append(fbs_sector_subset)
        else:
            fxn = primary_config.get("clean_fbs_df_fxn")
            if callable(fxn):
            ...
            log.info(f"Append {primary_source} to FBS list")
